from src.Check_Duplicate import API_check_not_duplicate
from src.Check_Category import API_check_category

from typing import Tuple, List
import csv
import re
from src.pororoocr import PororoOcr
...
def API_Document_Verify(image_path: str,
    ...
    with ThreadPoolExecutor() as executor:
        ...
        future_category = executor.submit(
            API_check_category, ocr_text, document_description)
    ...
    if (-1 in [check_category_result, check_duplicate_result]):
        return (0, 0, "예상치 못한 에러가 발생하였습니다. 다시 시도해주세요", None), ocr_text

    elif (check_duplicate_result and check_category_result):
        return (1, 1, "Success", category), ocr_text

    elif (not check_duplicate_result and check_category_result):
        return (1, 0, "과거 존재하는 자료이거나, 저작권이 존재하는 자료입니다.", None), ocr_text

    elif (not check_category_result and check_duplicate_result):
        return (0, 1, "해당 문서의 카테고리가 올바르지 않습니다.", None), ocr_text

    else:
        return (0, 0, "카테고리 검사 및 자료 유효성 검사를 통과하지 못하였습니다.", None), ocr_text
